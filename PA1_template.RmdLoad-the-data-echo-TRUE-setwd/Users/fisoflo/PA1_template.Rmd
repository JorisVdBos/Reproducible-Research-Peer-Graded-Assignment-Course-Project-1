...
intervale_max_steps<-steps_interval[which.max(steps_interval$average_steps),]$interval
intervale_max_steps
...
complete_data[NA_index, ]$steps<-unlist(lapply(NA_index, FUN=function(NA_index){
                steps_interval[activity[NA_index,]$interval==steps_interval$interval,]$average_steps
                }))
...
steps_interval_weekdays <- aggregate(weekdays_data$steps, by=list(interval=weekdays_data$interval), FUN=mean)

#Calculating the average number of steps for weekends
weekends_data <- complete_data[!complete_data$is_weekday,]
steps_interval_weekends <- aggregate(weekends_data$steps, by=list(interval=weekends_data$interval), FUN=mean)

#Adding columns names
colnames(steps_interval_weekdays) <- c("interval", "average_steps")
colnames(steps_interval_weekends) <- c("interval", "average_steps")

#Adding a column for "day"
steps_interval_weekdays$day <- "Weekday"
steps_interval_weekends$day <- "Weekend"
